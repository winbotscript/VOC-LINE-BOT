<?php
    ...
    function insertComplaintData($conn, $namedDataArray){
        ...
        foreach($namedDataArray as $row){
            ...
            $sent_date = (int)$row['วันที่คำร้องส่งถึง กฟภ.'];
            $received_date = (int)$row['วันที่รับข้อร้องเรียน'];
            $settlement_date = (int)$row['วันที่ปิดข้อร้องเรียน'];
            ...
            $sent_date_formated = date('d-m-Y H:m', PHPExcel_Shared_Date::ExcelToPHP($sent_date));
            $received_date_formated = date('d-m-Y H:m', PHPExcel_Shared_Date::ExcelToPHP($received_date));
            $settlement_date_formated = date('d-m-Y H:m', PHPExcel_Shared_Date::ExcelToPHP($settlement_date));
            ...
            $stmt->bind_param("isssssssssssssi",$count_complaint, $main_office,
                    ...
                    $sent_date_formated,$received_date_formated,$settlement_date_formated,
